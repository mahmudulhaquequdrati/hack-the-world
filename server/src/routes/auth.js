 ...
 *     description: |
 *       Refresh an existing JWT token to extend the session. Useful for maintaining user sessions without requiring re-login.
 *
 *       **🔒 Security Features:**
 *       - Validates existing token structure
 *       - Checks user account status
 *       - Generates new token with fresh expiration
 *       - Maintains same user permissions
 *
 *       **⚡ Usage Notes:**
 *       - Can refresh expired tokens (within reason)
 *       - User must still exist and be active
 *       - Returns new token with same expiration duration
 ...
 *           example:
 *             token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2NGExYjJjM2Q0ZTVmNjc4OTAxMjM0NSIsImlhdCI6MTY0MjUxMjAwMCwiZXhwIjoxNjQzMTE2ODAwfQ.signature'
 *     responses:
 *       200:
 ...
 *     description: |
 *       Validate a JWT token and return information about its validity and the associated user. This endpoint is useful for client-side token validation and user session management.
 *
 *       **🔒 Security Features:**
 ...
 *
 *       **⚡ Performance:**
 *       - Fast validation without database-heavy operations
 *       - Efficient token parsing and verification
 *       - Minimal response payload for quick processing
 *
 *       **🛡️ Error Handling:**
 *       - Graceful handling of invalid tokens
 *       - No error throwing for missing tokens
 ...
 *     responses:
 *       200:
 *         description: Token validation result (always returns 200)
 *         content:
 *           application/json:
 *             schema:
 *               oneOf:
